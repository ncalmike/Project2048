...
public class BoardManager {
      ...
      // RETURNS ARRAY OF BoardTiles VALUES AS A DELIMITED STRING
      public String getTilesValues(){
            ...
            for(int i = 0; i < NUM_ROWS; i++){
                  for(int j = 0; j < NUM_COLUMNS; j++){
                        ...
                        if(j < NUM_COLUMNS - 1) // ADD COMMAS AS DELIMITERS EXEPT LAST VALUE IN j LOOP
                              fromAry.append(",");
                  }
                  if(i < NUM_ROWS - 1) // ADD SEMICOLONS AS DELIMITERS EXEPT LAST VALUE IN i LOOP
                        fromAry.append(";");
            }
      ...
      public String alignEast(boolean addNewTile){
            ...
            for(int k = 0; k < NUM_ROWS; k++){
                  ...
                  boolean combined        = combineEast(k);
                  moveEast(k);
                  qualified                     = qualified || firstMove || combined;
            }
            isBoardFull(qualified);
      ...
      public boolean moveEast(int index){
            ...
            for(int i = NUM_COLUMNS - 1; i >= 0 ; i--){
                  if(BoardTiles[index][i].getValue() > 0){
                        while(i < NUM_COLUMNS - 1 && canMove(BoardTiles[index][i], BoardTiles[index][i+1])){
      ...
      public boolean combineNorth(int index){
            ...
            for(int i = 1; i < NUM_ROWS; i++){
                  if(tileHasValue(BoardTiles[i][index]) && canCombine(BoardTiles[i-1][index], BoardTiles[i][index])){
      ...
      public boolean combineEast(int index){
            ...
            for(int i = NUM_COLUMNS - 2; i >= 0 ; i--){
                  if(tileHasValue(BoardTiles[index][i]) && canCombine(BoardTiles[index][i+1], BoardTiles[index][i])){
